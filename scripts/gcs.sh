...
for mapping in "${MAPPINGS[@]}"; do
  ...
  # 1. Temporary pull subscription. Capture stderr so we see WHY it fails.
  CREATE_ERR="$TMPDIR/create-${TOPIC}.err"
  if ! gcloud pubsub subscriptions create "$SUB" \
        --topic="$TOPIC" \
        --project="$PROJECT" \
        --ack-deadline=20 \
    ...
    echo "  FAIL: could not create subscription on topic '$TOPIC'. gcloud said:"
    sed 's/^/    /' "$CREATE_ERR"
    FAIL=$((FAIL+1))
    FAILED_CASES+=("$TOPIC (subscription create failed)")
    echo
